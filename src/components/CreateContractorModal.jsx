...
function CreateContractorModal({ onClose, onSave }) {
  ...
  useEffect(() => {
    // Load one existing contractor to get field template
    const loadFieldTemplate = async () => {
      try {
        const snapshot = await getDocs(collection(db, 'contractors'));
        
        if (snapshot.docs.length > 0) {
          // Use first contractor as template
          const templateContractor = snapshot.docs[0].data();
          
          // Create empty form with all fields from template
          const emptyForm = {};
          Object.keys(templateContractor).forEach(key => {
            ...
            const value = templateContractor[key];
          ...
          console.log(`📋 Loaded field template from existing contractor (${Object.keys(emptyForm).length} fields)`);
          setFormData(emptyForm);
        } else {
          // No contractors exist - use basic template
          console.log('📋 No existing contractors, using basic template');
          setFormData({
            companyName: '',
            contactName: '',
            phone: '',
  ...
  const handleCreate = async () => {
    ...
    Object.keys(formData).forEach(key => {
      ...
      if (typeof value === 'boolean') {
        emptyContractor[key] = false;
      } else if (typeof value === 'object' && value !== null) {
        emptyContractor[key] = Array.isArray(value) ? [] : {};
      } else {
        emptyContractor[key] = '';
      }
    });
  ...
  const getFieldsToDisplay = () => {
    ...
    const fields = [];
    const basicFields = ['companyName', 'name', 'company', 'contactName', 'phone', 'email', 'isActive', 'notes'];
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
                <div className="form-group" style={{ flex: 1, margin: 0 }}>
                  ...
                  <input
                    ...
                    placeholder="e.g., licenseNumber, insuranceExpiry"
                    onKeyPress={(e) => {
                      if (e.key === 'Enter') {
